...
SelectMgr.prototype.getHeads = function () {
	...
	ret.push(new TableHead("", "验收时间"));
	ret.push(new TableHead("", "验收数量"));
	ret.push(new TableHead("", "单价"));
	ret.push(new TableHead("", "验收人"));	
	this._heads = ret;	
	return ret;
};
...
SelectMgr.prototype.buildRow = function (data, parentPath, ret) {
	...
	for (i = 0; i < len; i++) {
		...
		ret.push("<tr agid='", item.id, "'>");
			...
			ret.push("<td>", item.check_equipment_number, "</td>");
...
SelectMgr.prototype.initBody = function () {
	...
	$("a[excel_export]").click(function() {
		...
		$("a[excel_export]").attr("href", "select/check/export?"+arr);
	});
};
...
SelectMgr.prototype.initData = function () {
	...
	$.get("select/check/all", this.getSearchCriteria(), function (data) {
