...
EquAcceptMgr.prototype.dataTableOptions = function () {
	...
	opt.fnDrawCallback = function () {
		...
		$btn = $("#" + piThis.tableId + " tbody td button[" + piThis.btnAccept + "]");
...
EquAcceptMgr.prototype.resetStatus = function (key) {
	...
	$("button[status-switch]").each(function (i, btn) {
		...
		if ($btn.attr("key") == key) {
			if ($btn.is(".am-btn-warning")) {
				flag = false;
			} else {
				$btn.addClass("am-btn-warning");
			}
		} else {
			if ($btn.is(".am-btn-warning")) $btn.removeClass("am-btn-warning");
		}
		return true;
	});
...
function on_accept_equipment_load() {
	//initQuickMenus("equipment-accept");

	var mgr = acceptEquipmentsMgr();
	mgr.initialize();
