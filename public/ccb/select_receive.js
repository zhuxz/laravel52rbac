...
SelectMgr.prototype.getEquEditBody = function (_token, act, id) {
	...
	html.push("<form id='searchForm' class='am-form am-form-horizontal' action=''>");
		...
		html.push("<div class='am-form-group'>");	
			...
			//设备领用人
			html.push("<div class='am-u-sm-4'>");
				html.push("<input class='modal-text' type='text' name='receive_opu_name' placeholder='设备领用人' title='设备领用人' />");
			html.push("</div>");
		...
		html.push("<div class='am-form-group'>");	
			//设备使用人
			html.push("<div class='am-u-sm-4'>");
				html.push("<input class='modal-text' type='text' name='user_name' placeholder='设备使用人' title='设备使用人' />");
			html.push("</div>");

			//设备资产编号
			html.push("<div class='am-u-sm-4'>");
				html.push("<input class='modal-text' type='text' name='financial_code' placeholder='设备资产编号' title='设备资产编号' />");
			html.push("</div>");

			//设备序列号
			html.push("<div class='am-u-sm-4'>");
				html.push("<input class='modal-text' type='text' name='serials_code' placeholder='设备序列号' title='设备序列号' />");
			html.push("</div>");
		html.push("</div>");
...
SelectMgr.prototype.getHeads = function () {
	var ret = [];
	ret.push(new TableHead("", "所属分行"));
	ret.push(new TableHead("", "所属网点\部门"));
	...
	ret.push(new TableHead("", "使用人"));
	ret.push(new TableHead("", "领用人"));
	//ret.push(new TableHead("", "用途"));
	//ret.push(new TableHead("", "出库日期"));
	//ret.push(new TableHead("", "领用日期"));
	//ret.push(new TableHead("", "调拨日期"));
	ret.push(new TableHead("", "是否出库"));
	ret.push(new TableHead("", "是否领用"));
	ret.push(new TableHead("", "是否调拨"));
	this._heads = ret;	
	return ret;
};
...
SelectMgr.prototype.buildRow = function (data, parentPath, ret) {
	...
	for (i = 0; i < len; i++) {
		item = data[i];
		if (item.cnt) item.check_equipment_number = item.cnt;
		ret.push("<tr agid='", item.id, "'>");
			...
			ret.push("<td>", item.receive_user, "</td>");
			//ret.push("<td>", item.purposes_name, "</td>");
			
			/*if( item.allot_date != '' ){
				ret.push("<td>", (parseDate(item.allot_date)).Format("yyyy-MM-dd"), "</td>");
			}else{
				ret.push("<td></td>");
			}
			
			if( item.receive_date != '' ){
				ret.push("<td>", (parseDate(item.receive_date)).Format("yyyy-MM-dd"), "</td>");
			}else{
				ret.push("<td></td>");
			}

			if( item.pool_allot_date != '' ){
				ret.push("<td>", (parseDate(item.pool_allot_date)).Format("yyyy-MM-dd"), "</td>");
			}else{
				ret.push("<td></td>");
			}*/
			
			ret.push("<td>",item.whether_out,"</td>");
			ret.push("<td>",item.whether_receive,"</td>");
			ret.push("<td>",item.whether_allot,"</td>");
		ret.push("</tr>");
	}
...
SelectMgr.prototype.initBody = function () {
	...
	$("a[excel_export]").click(function() {
		...
		$("a[excel_export]").attr("href", "select/receive/export?"+arr);
	});
};
...
SelectMgr.prototype.initData = function () {
	...
	$.get("select/receive/all", this.getSearchCriteria(), function (data) {
