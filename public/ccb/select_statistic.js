...
SelectMgr.prototype.getHeads = function () {
	...
	ret.push(new TableHead("", "入库数"));
	ret.push(new TableHead("", "单价"));
	ret.push(new TableHead("", "入库日期"));
	ret.push(new TableHead("", "入库经办人"));
	//ret.push(new TableHead("", "验收日期"));
	//ret.push(new TableHead("", "验收经办人"));
	//ret.push(new TableHead("", "验收数"));
	//ret.push(new TableHead("", ""));
	ret.push(new TableHead("", "是否录入资产编号"));
	//ret.push(new TableHead("", "是否验收"));
	this._heads = ret;	
	return ret;
};
...
SelectMgr.prototype.buildRow = function (data, parentPath, ret) {
	...
	for (i = 0; i < len; i++) {
		...
		ret.push("<tr agid='", item.id, "'>");
			if (item.localInCount) item.stock = item.localInCount;
			if (item.quantity) item.equipment_total = item.quantity;
			ret.push("<td>", item.contract_code, "</td>");
			//ret.push("<td>",item.storage_name,"</td>");
			ret.push("<td>省分行仓库</td>");
			...
			ret.push("<td>",item.whether_financial_code,"</td>");
			//ret.push("<td>",item.whether_accept,"</td>");
			/*ret.push("<td>", item.real_name, "</td>");
			if(item.financial_code){
				ret.push("<td>资产编号已录入<a>查看资产编号</a></td>");
			}else{
				ret.push("<td>资产编号未录入</td>");
			}*/
		ret.push("</tr>");
	}
...
SelectMgr.prototype.initBody = function () {
	...
	$("a[excel_export]").click(function() {
		...
		$("a[excel_export]").attr("href", "select/warehouse/export?"+arr);
	});
};
...
SelectMgr.prototype.initData = function () {
	...
	/*$.get("select/warehouse/all", this.getSearchCriteria(), function (data) {
		piThis.cacheData(data);
